# This script changes SAF codes to JAC item numbers and formats data to final SAF dataset ready for combining with Ags (census data) in the next script.
# This script is based on code in B9 of the SAS June project (\\s0177a\datashare\seerad\ags\census\branch1\NewStructure\Surveys\June\Main\JUNE CENSUS PROJECT - 2021 Provisional Scott)
...
# Reformat SAF data to combine the areas of all entries with the same land use code on each holding to a single record. One line per non-zero land use item per holding
...
# Some of these will probably change yearly
...
cens_wide <-
  data.frame(lapply(cens_wide, function(x) {
    unlist((as.character(
      x
    )))
  }))
...
cens_wide2 <-
  left_join(cens_wide, extra_fields, by = c("parish", "holding"))

cens_wide3 <-
  left_join(cens_wide2, extra_item185, by = c("parish", "holding"))


cens_wide_final <-
  left_join(cens_wide3, brns, by = c("parish", "holding")) # check that this hasn't added rows
