...
# Insert error descriptions into tables -----------------------------------


#main validations
#create list of validations vector character
main_validations_desc <- as.list(paste(main_validation_list, "desc", sep="_"))
#change name of each list to error names
names(main_validations_desc) <- main_validation_list
main_validations_desc <-  lapply(main_validations_desc, function(x)get(x))
#single dataframe of errors and error descriptions
main_validations_desc <- bind_rows(main_validations_desc)
#change error column headings to error + description
colnames(v_list)[colnames(v_list) %in% colnames(main_validations_desc)] <-  main_validations_desc %>% select(any_of(names(v_list)))
#colnames(labour_holding_list)[colnames(labour_holding_list) %in% colnames(main_validations_desc)] <-  main_validations_desc %>% select(any_of(names(labour_holding_list)))
colnames(v_nplist_trim)[colnames(v_nplist_trim) %in% colnames(main_validations_desc)] <-  main_validations_desc %>% select(any_of(names(v_nplist_trim)))


#validation summary:create error description column and change error rownames to error descriptions
v_summary_all$error_description <-  paste(v_summary_all$error, "desc", sep="_")
relevant_desc <- v_summary_all$error_description[grep("^err", v_summary_all$error_description)]
v_summary_all$error_description[v_summary_all$error_description%in%relevant_desc] <- lapply(relevant_desc, function(x)get(x))

#as above for summary broken down by priority 
v_summary_prioritised$error_description <-  paste(v_summary_prioritised$error, "desc", sep="_")
relevant_desc <- v_summary_prioritised$error_description[grep("^err", v_summary_prioritised$error_description)]
v_summary_prioritised$error_description[v_summary_prioritised$error_description%in%relevant_desc] <- lapply(relevant_desc, function(x)get(x))


#context errors
#create list of validations vector character
all_context_desc <- paste(context_error_list, "desc", sep="_")
#create list
all_context_desc <-as.list(all_context_desc)  
#change name of each list to error names
names(all_context_desc) <- context_error_list
all_context_desc <-  lapply(all_context_desc, function(x)get(x))
#single dataframe of errors and error descriptions
all_context_desc <- bind_rows(all_context_desc)
#change error column headings to error + description
colnames(cv_list)[colnames(cv_list) %in% colnames(all_context_desc)] <-  all_context_desc %>% select(any_of(names(cv_list)))

#validation summary:change error rownames to error descriptions
cv_summary$error[cv_summary$error%in%colnames(all_context_desc)] <- all_context_desc %>% select(any_of(cv_summary$error))


#module
#create list of validations vector character
all_module_validations_desc <- as.list(paste(merr_list, "desc", sep="_"))
#create list
...
#change error column headings to error + description
colnames(mv_list)[colnames(mv_list) %in% colnames(all_module_validations_desc)] <-  all_module_validations_desc %>% select(any_of(names(mv_list)))
#validation summary:change error rownames to error descriptions
mv_summary$error[mv_summary$error %in% colnames(all_module_validations_desc)] <-  all_module_validations_desc %>% select(any_of(mv_summary$error))
...
mv_summary$error <- as.character(mv_summary$error)
cv_summary$error <- as.character(cv_summary$error)
