...
keep_perr_vars <- function(x){
  perr_x <<- errors %>% select(parish, holding, survtype, submisType, land_data, saf_data, any_of(x))    
  name_x <<- sub("\\_.*", "", substitute(x))
}

#function to remove columns where error = 0
remove_zero <- function(x){select(x, where(~any(.!= 0)))}
remove_NA <- function(x){select(x, where(~any(!is.na(.))))}
...
perr15_vars <- c("perr15", total_land, rps_total_area, total_area, ototal_land, vototal_land)
...
JAC23 <- combined_JAC %>% group_by(parish, holding) %>% 
  select (parish, holding,  survtype, submisType, any_of(as.character(context_items)), other_data, land_data, saf_data, contains("rps")  )
...
prev_yr<- prev_yr %>% group_by(parish, holding) %>% 
  select (parish, holding, survtype, ags_madeup, saf_madeup, #submisType,
          any_of(as.character(context_items)), other_data, land_data, survdata )
...
all_context_errors<- all_context_errors %>% 
  ...
  #Land check - If total area has gone up or down by 10% and there has been an increase of grass greater than 5 years in the last year that does not account for the total of last year's rough grazings, fallow, other land and the difference in total land use area change.
    ...
    perr1 = case_when(0.9*get(oarea_own)< get(total_area) & 
                         ...
                         othgrass - oothgrass > ograss+ get(orough_graze)+ (get(fland_lte5) + get(fland_gt5)) +get(oother_land) + 20 + max(get(total_area)-get(ototal_area), 0)~ 1,
                      TRUE ~ 0),
...
perr2 <-perr2 %>%  mutate(error=ifelse(perr2== 1, "Error", "No Error"),
                           difference1 = ifelse(get(total_pig)-get(ototal_pig) < 0,
                                               -0.5*(get(total_pig)-get(ototal_pig)), get(total_pig)-get(ototal_pig)),
                          difference2 = ifelse(get(total_pig)-get(vototal_pig) < 0,
                                               -0.5*(get(total_pig)-get(vototal_pig)), get(total_pig)-get(vototal_pig))
...
perr2 <- perr2 %>% filter(abs(difference1) & abs(difference2)  >=100)
...
perr3 <-perr3 %>%  mutate(error=ifelse(perr3== 1, "Error", "No Error"),
                          difference = ifelse(get(total_poultry) -get(ototal_poultry) < 0, 
                                              -0.5*get(total_poultry)-get(ototal_poultry), get(total_poultry)-get(ototal_poultry))
)
...
perr7 <-perr7 %>%  mutate(error=ifelse(perr7== 1, "Error", "No Error"),
                          difference1 = ifelse(get(total_sheep) -get(ototal_sheep) < 0, 
                                              -0.5*(get(total_sheep)-get(ototal_sheep)), get(total_sheep)-get(ototal_sheep)),
                          difference2 = ifelse(get(total_sheep) -get(vototal_sheep) < 0, 
                                                                   -0.5*(get(total_sheep)-get(vototal_sheep)), get(total_sheep)-get(vototal_sheep)                    )
)
#filter for errors
...
perr7 <- perr7 %>% filter(abs(difference1)>=500 & abs(difference2>=500))
...
perr9 <-perr9 %>%  mutate(error=ifelse(perr9== 1, "Error", "No Error"),
                          difference = ifelse(get(soft_fruits)  -get(osoft_fruits)  < 0, 
                                              -0.5*(get(soft_fruits) -get(osoft_fruits) ), get(soft_fruits) -get(osoft_fruits))
)
...
context_error_list <- c("perr1", "perr2", "perr3", "perr4", "perr5", "perr6",
...
relevant_errors <- names(errors)[names(errors) %in% context_error_list]
...
context_error_summary <- error_filter %>% summarize(across(everything(), sum, na.rm = TRUE)) %>% ungroup()
...
#clean environment
rm(combined_JAC, prev_2yr, prev_yr, JAC23, context_errors)
##----Output for Ops--------------------------------------------------------------------------------------------
...
errors$total_errors_per_case <- rowSums(errors[names(errors) %in% context_error_list], na.rm=TRUE)

#filter where total error per case !=0 
errors<- errors %>% filter(total_errors_per_case !=0)

#select relevant columns
error_cases <- errors %>% select(parish, holding, starts_with("perr"), total_errors_per_case)                                      

#single dataframe of holdings with validation errors
holding_list <- bind_rows(error_cases) 
#remove columns with NAs only from each dataframe in holding_list
holding_list <- remove_NA(error_cases)
