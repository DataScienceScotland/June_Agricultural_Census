...
#err1
all_JAC_form$err1 <-  ifelse(abs(round(all_JAC_form[total_area], digits = 2) - round(all_JAC_form[area_own]+all_JAC_form[area_rent], digits = 2)) >= 3,  1, 0)
all_JAC_form$err1_diff <-  ifelse(all_JAC_form$err1 == 1, 
                                      abs(rowSums((all_JAC_form[area_own] + all_JAC_form[area_rent])- all_JAC_form[total_area])),0 )

#Total area is zero but area own or area rented >0
#err37
all_JAC_form$err37 <- ifelse((all_JAC_form[total_area] == 0 |  is.na(all_JAC_form[total_area])) & (all_JAC_form[area_own]| all_JAC_form[area_rent] > 0), 1, 0)
...
all_JAC_form$err38 <- ifelse((all_JAC_form[total_area] == 0 |  is.na(all_JAC_form[total_area])) & 
                               ...
                               all_JAC_form[area_rent] == 0 |  is.na(all_JAC_form[area_rent]), 1, 0)

#Area rented not equal to RP&S value by 3 or more ha
# #all_JAC_form$err56 <- ifelse(abs(round(all_JAC_form[area_rent], digits = 2) - round(all_JAC_form[rps_rent], digits = 2))>=3, 1, 0)
# #all_JAC_form$err56_diff <- ifelse(round(rowSums(abs(all_JAC_form[area_rent] - all_JAC_form[rps_rent])>=3), digits = 2), 
#                                   rowSums(abs(all_JAC_form[area_rent] - all_JAC_form[rps_rent])), 0)


#Area rented greater/less than RP&S value by 30ha or more
#err57
all_JAC_form$err57 <- ifelse(abs(round(all_JAC_form[area_rent], digits = 2) - round(all_JAC_form[rps_rent], digits = 2)) >=30, 1, 0)
all_JAC_form$err57_diff <-  ifelse(all_JAC_form$err57 == 1, 
                                   rowSums(abs(all_JAC_form[area_rent] - all_JAC_form[rps_rent])), 0)


#Area owned not equal to RP&S value by 3 or more ha
# all_JAC_form$err58 <- ifelse(abs(round(all_JAC_form[area_own], digits = 2) - round(all_JAC_form[rps_own], digits = 2))>=3, 1, 0)
# all_JAC_form$err58_diff <-  ifelse(round(rowSums(abs(all_JAC_form[area_own] - all_JAC_form[rps_own])>=3), digits = 2), 
...
#err2
all_JAC_form$err2 <- ifelse(round(all_JAC_form[seas_total_area_in], digits = 2) == round(all_JAC_form[area_rent], digits = 2)
                            & (all_JAC_form[seas_total_area_in] & all_JAC_form[area_rent]> 0) & all_JAC_form$land_data == "ags",  1, 0)
...
all_JAC_form$err43_diff <- ifelse(all_JAC_form$err43==1,
                                  rowSums(all_JAC_form[seas_area_rent_out] - all_JAC_form[total_area]), 0)
...
all_JAC_form$err7 <-  ifelse(!is.na(all_JAC_form[all_veg_saf]) & !is.na(all_JAC_form[all_veg_open]), ifelse(round(all_JAC_form[all_veg_saf], digits = 2) != 
                                                                                                                round(all_JAC_form[all_veg_open], digits = 2), 1, 0), 0)
all_JAC_form$err7_diff <- ifelse(all_JAC_form$err7 ==1, abs(round(rowSums(all_JAC_form[all_veg_saf] - all_JAC_form[all_veg_open]), digits = 2)), 0)
...
all_JAC_form$err8_diff <- ifelse(all_JAC_form$err8 == 1, abs(rowSums(all_JAC_form[all_fruit_saf]- all_JAC_form$total3)),0)
...
all_JAC_form$err10 <-  ifelse(!is.na(all_JAC_form[all_flow_bulb]) & !is.na(all_JAC_form$total4), ifelse(round(all_JAC_form[all_flow_bulb], digits = 2) != 
...
# 
# all_JAC_form$err11open_diff <- ifelse(all_JAC_form$err11==1, 
#abs(rowSums(all_JAC_form[tot_open_plastic] - all_JAC_form$total5)),0)

#   
# all_JAC_form$err11solid_diff <- ifelse(all_JAC_form$err11 == 1, abs(rowSums(all_JAC_form[tot_solid_glass] - all_JAC_form$total6)), 0)
...
all_JAC_form$err11open_diff <- ifelse(all_JAC_form$err11 == 1, abs(rowSums(all_JAC_form[tot_open_plastic] - all_JAC_form$total5b)), 0)

all_JAC_form$err11solid_diff <- ifelse(all_JAC_form$err11 == 1, abs(rowSums(all_JAC_form[tot_solid_glass] - all_JAC_form$total6b)), 0)
...
all_JAC_form$err12_diff <- ifelse(all_JAC_form$err12 == 1, abs(rowSums((all_JAC_form[tot_open_plastic] + all_JAC_form[tot_solid_glass]) - (all_JAC_form$total5b + all_JAC_form$total6b))), 0)
...
all_JAC_form$err45 <- ifelse(!is.na(all_JAC_form[total_land]), ifelse(all_JAC_form$land_data == "ags" & abs(all_JAC_form[total_land] - all_JAC_form$total13)>=3, 1, 0), 0)
all_JAC_form$err45_diff <- ifelse(all_JAC_form$err45==1, abs(rowSums(all_JAC_form[total_land] - all_JAC_form$total13)),  0)


#Total at end of land use section equal to total area 

all_JAC_form$err46 <- ifelse(!is.na(all_JAC_form[total_land]) & !is.na(all_JAC_form[total_area]), ifelse(all_JAC_form$land_data == "ags" & abs(all_JAC_form[total_land] - all_JAC_form[total_area])>=3, 1, 0), 0)
all_JAC_form$err46_diff <- ifelse(all_JAC_form$err46 == 1, abs(rowSums(all_JAC_form[total_land] - all_JAC_form[total_area])), 0)
...
all_JAC_form$err13_diff <- ifelse(all_JAC_form$err13 == 1,  abs(rowSums(all_JAC_form[total_pig] - all_JAC_form$total10)), 0)
...
all_JAC_form$err15_diff <- ifelse(all_JAC_form$err15 == 1, abs(rowSums(all_JAC_form[total_sheep] - all_JAC_form$total9)), 0)
...
all_JAC_form$err17_diff <- ifelse(all_JAC_form$err17 == 1,  abs(rowSums(all_JAC_form[total_poultry] - all_JAC_form$total11)), 0)
...
#err24 = err38 in SAS
all_JAC_form$err24 <-  ifelse(all_JAC_form[other_livestock] > 0 &(is.na(all_JAC_form[other_livestock_txt])| all_JAC_form[other_livestock_txt] ==""),  1, 0)
...
all_JAC_form$err60 <- ifelse(all_JAC_form[legal_fin_resp] ==0 & all_JAC_form[other_legal_return] == 0 |
...
all_JAC_form$err51 <- ifelse((all_JAC_form[occupier1_year] !=0 & all_JAC_form[occupier1_year] < 1923 & all_JAC_form[occupier1_year]>2008)| 
                               (all_JAC_form[occupier2_year]!=0 & all_JAC_form[occupier2_year] < 1923 & all_JAC_form[occupier2_year]>2008), 1,0)
...
all_JAC_form$err32_diff <- ifelse(all_JAC_form$err32 == 1, rowSums(abs(all_JAC_form[total_labour] - all_JAC_form$total12)), 0)
...
d15 <- c(#glass_tom_tot, (item87 no longer collected from form- SAF dataset only collects item2707 (protected crops tomatoes solid floor))
  glass_tom_solid, 
...
fruit_holdings$nonreg <- fruit_holdings[labour_non_fam_non_reg] + fruit_holdings[labour_agri_contract] + fruit_holdings[labour_non_EU_migrant] +fruit_holdings[labour_EU_migrant]
...
#Sum labour by brn
brn_fruit <- fruit_holdings %>% ungroup() %>% select(brn, sumfruit, slr_fruit, lab_returned, all_of(c(labour_cas_f, labour_cas_m))) %>% group_by(brn) %>% filter(brn >0 ) %>% arrange(brn) 
...
large_fruit <-fruit_holdings %>% filter(sumfruit>2 & survtype != "SAF_only") %>% arrange(brn, by_group = FALSE)
...
check_labour <- check_labour %>%  select(parish, holding, survtype, submisType, brn, land_data, saf_data, sumfruit, sumfruit_sum, slr_fruit, slr_fruit_sum, lab_returned, lab_returned_sum, diff_lab, diff_lab_nobrn, labour_cas_f, labour_cas_m)
...
JAC_validation_errors <- all_JAC_form %>% select(parish, holding, submisType, any_of(all_validations)) %>%  filter(if_any(starts_with("err"), ~ . !=0))
JAC_validation_error_summary <- JAC_validation_errors%>% ungroup() %>% group_by(submisType) %>%   select(starts_with("err")) %>% summarize(across(everything(), sum, na.rm = TRUE))
JAC_validation_error_summary <- cbind(JAC_validation_error_summary, total_cases_with_errors = rowSums(JAC_validation_error_summary[names(JAC_validation_error_summary) %in% all_validations], na.rm=TRUE))
JAC_validation_error_summary <- as.data.frame(t(JAC_validation_error_summary)) %>% 
  janitor::row_to_names(1)
colnames(JAC_validation_error_summary) <- paste("SubmisType", colnames(JAC_validation_error_summary) )

#R Markdown tables
##----err 1----------------------------------------------------------------------------------------------
##@knitr err1

err1 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(area_own, area_rent, total_area)), err1, err1_diff)     
err1 <-err1 %>%  mutate(error=ifelse(err1== 1 | is.na(err1), "Error", "No Error")) %>% filter(error == "Error")

##----err 37----------------------------------------------------------------------------------------------
##@knitr err37
err37 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(area_own, area_rent, total_area)), err37)     
...
err38 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(area_own, area_rent, total_area)), err38)     
...
err57<- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(rps_rent, area_rent)), err57, err57_diff)     
err57 <-err57 %>%  mutate(error=ifelse(err57== 1 | is.na(err57), "Error", "No Error")) %>% filter(error == "Error")
...
err3 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(seas_total_area_in, seas_rent_diff_in, seas_rent_same_in)), err3, err3_diff)     
err3 <- err3 %>%  mutate(error=ifelse(err3== 1 | is.na(err3), "Error", "No Error")) %>% filter(error == "Error")

##----err 43----------------------------------------------------------------------------------------------
##@knitr err43

err43 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(seas_area_rent_out, total_area)), err43, err43_diff)     
...
err4 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(other_crops, other_crops_text)), err4)     
err4 <-err4 %>%  mutate(error=ifelse(err4== 1 | is.na(err4), "Error", "No Error"))%>% filter(error == "Error")

##----err 44----------------------------------------------------------------------------------------------
##@knitr err44

err44 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(other_crops, other_crops_text)), err44)     
err44 <-err44 %>%  mutate(error=ifelse(err44== 1 | is.na(err44), "Error", "No Error"))%>% filter(error == "Error")

##----err 5----------------------------------------------------------------------------------------------
##@knitr err5

err5 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total_crops_grass, rough_graze, woodland, other_land, total_land)), total7, err5, err5_diff)     
...
err7 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(all_veg_saf, all_veg_open)), err7, err7_diff)     
...
##----err 9----------------------------------------------------------------------------------------------
##@knitr err9
err9 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(soft_fruits, all_fruit_saf)), err9, err9_diff)     
...
err10 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(all_flow_bulb, total4)), err10, err10_diff)     
err10 <-err10 %>%  mutate(error=ifelse(err10== 1 |is.na(err10), "Error", "No Error"))%>% filter(error == "Error")

##----err 11----------------------------------------------------------------------------------------------
##@knitr err11
err11 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total5b, tot_open_plastic, total6b, tot_solid_glass)), err11, err11open_diff, err11solid_diff)
err11 <-err11 %>%  mutate(error=ifelse(err11== 1 | is.na(err11), "Error", "No Error"))%>% filter(error == "Error")

##----err 12----------------------------------------------------------------------------------------------
##@knitr err12
err12 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total5b, tot_open_plastic, total6b, tot_solid_glass)), err12, err12_diff)     
err12 <-err12 %>%  mutate(error=ifelse(err12== 1 | is.na(err12), "Error", "No Error"))%>% filter(error == "Error")

##----err 45----------------------------------------------------------------------------------------------
##@knitr err45
err45 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total_land, total_area, rps_total_area)), total13, err45, err45_diff)     
err45 <-err45 %>%  mutate(error=ifelse(err45== 1 | is.na(err45), "Error", "No Error"))%>% filter(error == "Error")

##----err 46----------------------------------------------------------------------------------------------
##@knitr err46
err46 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total_land, total_area, rps_total_area)), err46, err46_diff)     
...
err13 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total10, total_pig)), err13, err13_diff)     
err13 <-err13 %>%  mutate(error=ifelse(err13== 1 | is.na(err13), "Error", "No Error"))%>% filter(error == "Error")

##----err 14----------------------------------------------------------------------------------------------
##@knitr err14
#change total10_21 to total10 for 2023
err14 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total10)), err14)     
err14 <-err14 %>%  mutate(error=ifelse(err14== 1 | is.na(err14), "Error", "No Error"))%>% filter(error == "Error")

##----err 15----------------------------------------------------------------------------------------------
##@knitr err15
err15 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total9, total_sheep)), total9, err15, err15_diff)     
err15 <-err15 %>%  mutate(error=ifelse(err15== 1 | is.na(err15), "Error", "No Error"))%>% filter(error == "Error")

##----err 16----------------------------------------------------------------------------------------------
##@knitr err16
err16 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total9, total_sheep)), total9, err16)     
err16 <-err16 %>%  mutate(error=ifelse(err16== 1 | is.na(err16), "Error", "No Error"))%>% filter(error == "Error")

##----err 17----------------------------------------------------------------------------------------------
##@knitr err17
err17 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total11, total_poultry)), total11, err17, err17_diff)     
err17 <-err17 %>%  mutate(error=ifelse(err17== 1 | is.na(err17), "Error", "No Error"))%>% filter(error == "Error")

##----err 18----------------------------------------------------------------------------------------------
##@knitr err18
err18 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total11, total_poultry)), err18)     
err18 <-err18 %>%  mutate(error=ifelse(err18== 1 | is.na(err18), "Error", "No Error"))%>% filter(error == "Error")

##----err 19----------------------------------------------------------------------------------------------
##@knitr err19
err19 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(first_hens, moulted_hens, pullets, layer_chicks, table_chicks, cocks, broilers)), err19)     
err19 <-err19 %>%  mutate(error=ifelse(err19== 1 | is.na(err19), "Error", "No Error"))%>% filter(error == "Error")

##----err 20----------------------------------------------------------------------------------------------
##@knitr err20
err20 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(deer), err20)     
err20 <-err20 %>%  mutate(error=ifelse(err20== 1 | is.na(err20), "Error", "No Error"))%>% filter(error == "Error")

##----err 21----------------------------------------------------------------------------------------------
##@knitr err21
err21 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(alpacas, llamas, other_camelids)), err21)     
err21 <-err21 %>%  mutate(error=ifelse(err21== 1 | is.na(err21), "Error", "No Error"))%>% filter(error == "Error")

##----err 22----------------------------------------------------------------------------------------------
##@knitr err22
err22 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(hives)), err22)     
err22 <-err22 %>%  mutate(error=ifelse(err22== 1 | is.na(err22), "Error", "No Error"))%>% filter(error == "Error")

##----err 23----------------------------------------------------------------------------------------------
##@knitr err23
err23 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(other_livestock)), err23)  
err23 <-err23 %>%  mutate(error=ifelse(err23== 1 | is.na(err23), "Error", "No Error"))%>% filter(error == "Error")

##----err 24----------------------------------------------------------------------------------------------
##@knitr err24
err24 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(other_livestock, other_livestock_txt)), err24)  
err24 <-err24 %>%  mutate(error=ifelse(err24== 1 | is.na(err24), "Error", "No Error"))%>% filter(error == "Error")

##----err 47----------------------------------------------------------------------------------------------
##@knitr err47
err47 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(other_livestock, other_livestock_txt)), err47)  
...
##----err 27----------------------------------------------------------------------------------------------
##@knitr err27
err27 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(horse_pony, donkeys)), err27)
err27 <-err27 %>%  mutate(error=ifelse(err27== 1 | is.na(err27), "Error", "No Error"))%>% filter(error == "Error")

##----err 28----------------------------------------------------------------------------------------------
# ##@knitr err28
err28 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(goats_kids)), err28)
err28 <-err28 %>%  mutate(error=ifelse(err28== 1 | is.na(err28), "Error", "No Error"))%>% filter(error == "Error")

##----err 60----------------------------------------------------------------------------------------------
# ##@knitr err60
err60 <- all_JAC_form %>% select(parish, holding, survtype, submisType,
                                 land_data, saf_data, all_of(c(legal_fin_resp, other_legal_return)), err60)
err60 <-err60 %>%  mutate(error=ifelse(err60== 1 | is.na(err60), "Error", "No Error"))%>% filter(error == "Error")


##----err 29----------------------------------------------------------------------------------------------
##@knitr err29
err29 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(other_legal_return, occupier1_ft, occupier1_pt_gthalf, occupier1_pt_lthalf, occupier1_no_work,
...
##----err 52----------------------------------------------------------------------------------------------
##@knitr err52
err52 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(other_legal_return, occupier1_ft, occupier1_pt_gthalf, occupier1_pt_lthalf, occupier1_no_work,
...
##----err 53----------------------------------------------------------------------------------------------
##@knitr err53
err53 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(occupier2_legal_fin, occupier2_ft, occupier2_pt_gthalf, occupier2_pt_lthalf, occupier2_no_work)), err53)
err53 <-err53 %>%  mutate(error=ifelse(err53== 1 | is.na(err53), "Error", "No Error"))%>% filter(error == "Error")


##----err 54----------------------------------------------------------------------------------------------
##@knitr err54
err54 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(occupier2_legal_fin, occupier2_ft, occupier2_pt_gthalf, occupier2_pt_lthalf, occupier2_no_work)), err54)
err54 <-err54 %>%  mutate(error=ifelse(err54== 1 | is.na(err54), "Error", "No Error"))%>% filter(error == "Error")


##----err 30----------------------------------------------------------------------------------------------
##@knitr err30
err30 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(occupier1_ft, occupier1_pt_gthalf, occupier1_pt_lthalf, occupier1_no_work)), err30)  
err30 <-err30 %>%  mutate(error=ifelse(err30== 1 | is.na(err30), "Error", "No Error"))%>% filter(error == "Error")

##----err 31----------------------------------------------------------------------------------------------
##@knitr err31
err31 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(occupier2_ft, occupier2_pt_gthalf, occupier2_pt_lthalf, occupier2_no_work)), err31)  
err31 <-err31 %>%  mutate(error=ifelse(err31== 1 | is.na(err31), "Error", "No Error"))%>% filter(error == "Error")

##----err 48----------------------------------------------------------------------------------------------
##@knitr err48
err48 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(occupier1_female, occupier1_male, occupier2_female, occupier2_male)), err48)  
err48 <-err48 %>%  mutate(error=ifelse(err48== 1 | is.na(err48), "Error", "No Error"))%>% filter(error == "Error")

##----err 49----------------------------------------------------------------------------------------------
##@knitr err49
err49 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(occupier1_female, occupier1_male, occupier1_ft, occupier1_pt_gthalf, occupier1_pt_lthalf, occupier1_no_work)), err49)  
err49 <-err49 %>%  mutate(error=ifelse(err49== 1 | is.na(err49), "Error", "No Error"))%>% filter(error == "Error")

##----err 50----------------------------------------------------------------------------------------------
##@knitr err50
err50 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(occupier1_female, occupier1_male, occupier1_ft, occupier1_pt_gthalf, occupier1_pt_lthalf, occupier1_no_work)), err50)  
err50 <-err50 %>%  mutate(error=ifelse(err50== 1 | is.na(err50), "Error", "No Error"))%>% filter(error == "Error")


##----err 51----------------------------------------------------------------------------------------------
##@knitr err51
err51 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(occupier1_year, occupier2_year)), err51)  
err51 <-err51 %>%  mutate(error=ifelse(err51== 1 | is.na(err51), "Error", "No Error"))%>% filter(error == "Error")

##----err 55----------------------------------------------------------------------------------------------
##@knitr err55
err55 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(occupier1_ft, occupier1_pt_gthalf, occupier1_pt_lthalf, occupier1_no_work, occupier1_male, occupier1_female,
...
##----err 32----------------------------------------------------------------------------------------------
##@knitr err32
err32 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total12, total_labour)), total12, err32)  
err32 <-err32 %>%  mutate(error=ifelse(err32== 1 | is.na(err32), "Error", "No Error"))%>% filter(error == "Error")


##----err 33----------------------------------------------------------------------------------------------
##@knitr err33
err33 <- all_JAC_form %>% select(parish, holding, survtype, submisType, land_data, saf_data, all_of(c(total12, total_labour)), err33)  
...
##----err 39----------------------------------------------------------------------------------------------
##@knitr err39
err39 <- check_labour %>% select(parish, holding, survtype, submisType, land_data, saf_data, brn, land_data, saf_data, sumfruit, sumfruit_sum, slr_fruit, slr_fruit_sum, lab_returned, lab_returned_sum, all_of(c(labour_cas_f, labour_cas_m)), err39)
err39 <-err39 %>%  mutate(error=ifelse(err39== 1 | is.na(err39), "Error", "No Error"))%>% filter(error == "Error")


##----err 40----------------------------------------------------------------------------------------------
##@knitr err40
err40 <- check_labour %>% select(parish, holding, survtype, submisType, land_data, saf_data,brn, land_data, saf_data, sumfruit, sumfruit_sum, slr_fruit, slr_fruit_sum, lab_returned, lab_returned_sum, all_of(c(labour_cas_f, labour_cas_m)), err40)
err40 <-err40 %>%  mutate(error=ifelse(err40== 1 | is.na(err40), "Error", "No Error"))%>% filter(error == "Error")

##----err 41----------------------------------------------------------------------------------------------
##@knitr err41
err41 <- check_labour %>% select(parish, holding, survtype, submisType, land_data, saf_data,brn, land_data, saf_data, sumfruit, sumfruit_sum, slr_fruit, slr_fruit_sum, lab_returned, lab_returned_sum, all_of(c(labour_cas_f, labour_cas_m)), err41)
err41 <-err41 %>%  mutate(error=ifelse(err41== 1 | is.na(err41), "Error", "No Error"))%>% filter(error == "Error")

##----err 42----------------------------------------------------------------------------------------------
##@knitr err42
err42 <- check_labour %>% select(parish, holding, survtype, submisType, land_data, saf_data,brn, land_data, saf_data, sumfruit, sumfruit_sum, slr_fruit, slr_fruit_sum, lab_returned, lab_returned_sum, all_of(c(labour_cas_f, labour_cas_m)), err42)
...
main_validations <- all_JAC_form %>% select(parish, holding, any_of(all_validations), err46_diff, err45_diff, err59_diff, err57_diff, err61_diff) 
#list of relevant errors to filter
relevant_errors <- names(main_validations[names(main_validations) %in% all_validations])
main_validations <- main_validations %>% filter(if_any(relevant_errors)!=0)


migrant_errors <- check_labour %>%  select(parish, holding, any_of(all_validations))
relevant_merrors <- names(migrant_errors[names(migrant_errors) %in% all_validations])
migrant_errors <- migrant_errors %>% filter(if_any(relevant_merrors)!=0)
...
priority <- c("err46_diff", "err46", "err45", "err45_diff", "err4", "err44", "err24", "err47", "err49", "err50", "err30", "err31",  "err60", "err54", "err53", "err48",
              "err51", "err55", "err11",  "err1", "err37", "err38","err61", "err61_diff", "err52")

#arrange columns according to work item priority order
main_validations<- main_validations %>% select(parish, holding, any_of(priority)) 
...
main_validations <- main_validations %>% arrange(desc(err46), desc(err46_diff), desc(err45), desc(err45_diff), 
                                                 desc(err4), desc(err44), 
                                                 desc(err24), desc(err47), desc(err49), desc(err50), 
                                                 desc(err30), desc(err31),  desc(err60),
...
main_validations$total_errors_per_case <- rowSums(main_validations[names(main_validations) %in% relevant_errors], na.rm=TRUE)
migrant_errors$total_errors_per_case <- rowSums(migrant_errors[names(migrant_errors) %in% relevant_merrors], na.rm=TRUE)

#filter where total error per case !=0 
main_validations <- main_validations %>% filter(total_errors_per_case !=0)
migrant_errors<- migrant_errors %>% filter(total_errors_per_case !=0)

#filter to select cases where err60 is the only error present
legal_only_error<- main_validations %>% filter(err60==1 & total_errors_per_case ==1)
                                          

#anti-join to remove cases where err60 is the only error present from total error cases 
main_validations <-anti_join(main_validations, legal_only_error, by=c("parish", "holding"))
...
non_priority_full <-  all_JAC_form %>% select(parish, holding, any_of(all_validations)) %>%  filter(if_any(starts_with("err"), ~ . !=0))
non_priority_full<-   anti_join(non_priority_full, main_validations, 
                            by = c("parish", "holding"))
non_priority_full$total_errors_per_case <- rowSums(non_priority_full[grep("err", names(non_priority_full))], na.rm=TRUE)
non_priority_full <- non_priority_full %>% filter(total_errors_per_case !=0)
...
non_priority_full <- as.data.frame(lapply(non_priority_full, as.numeric))

#adding priority and non-priority error counts
priority_error_count <- c("total_cases_with_priority_errors", nrow(main_validations))
non_priority_full_error_count <- c("total_cases_with_non_priority_full_errors", nrow(non_priority_full))


#TRIM List for  Ops-------------------------------------------------------------------------------------------
#trim down errors - remove errors that analysts  can correct without Ops intervention
trim_list <- c("err13", "err15", "err57", "err59", "err60", "err13", "err29", "err3", "err5", "err6", "err7", "err8", "err9", "err13", "err32", "err17" )

non_priority_trim <- non_priority_full %>%  select(-any_of(trim_list), -total_errors_per_case)

#calculate new total errors per case, after trimming error list
non_priority_trim$total_errors_per_case <- rowSums(non_priority_trim[grep("err", names(non_priority_trim))], na.rm=TRUE)
non_priority_trim<- non_priority_trim%>% filter(total_errors_per_case !=0)
non_priority_trim <- remove_zero(non_priority_trim)


non_priority_trim_error_count <- c("total_cases_with_non_priority_errors_trim", nrow(non_priority_trim))


#priority validation summary
priority_main_validations_summary <- main_validations %>% ungroup %>% select(-c(parish, holding, contains(c("diff", "total")))) %>%  summarize(across(everything(), sum, na.rm = TRUE))
priority_main_validations_summary <- cbind(priority_main_validations_summary, total_cases_with_errors = rowSums(priority_main_validations_summary[names(priority_main_validations_summary) %in% all_validations]))
priority_main_validations_summary <- priority_main_validations_summary %>% pivot_longer(cols= everything(), names_to = "error", values_to = "count")
priority_main_validations_summary <- priority_main_validations_summary %>% filter(count !=0)

#non_priority-full validation summary
non_priority_main_validations_summary <- non_priority_full %>% ungroup %>% select(-c(parish, holding, contains(c("diff", "total")))) %>%  summarize(across(everything(), sum, na.rm = TRUE))
non_priority_main_validations_summary <- cbind(non_priority_main_validations_summary, total_cases_with_errors = rowSums(non_priority_main_validations_summary[names(non_priority_main_validations_summary) %in% all_validations]))
non_priority_main_validations_summary <- non_priority_main_validations_summary %>% pivot_longer(cols= everything(), names_to = "error", values_to = "count")
non_priority_main_validations_summary <- non_priority_main_validations_summary %>% filter(count !=0)

#non_prirority trim validation summary
non_priority_trim_validations_summary <- non_priority_trim %>% ungroup %>% select(-c(parish, holding, contains(c("diff", "total")))) %>%  summarize(across(everything(), sum, na.rm = TRUE))
non_priority_trim_validations_summary <- cbind(non_priority_trim_validations_summary, total_cases_with_errors = rowSums(non_priority_trim_validations_summary[names(non_priority_trim_validations_summary) %in% all_validations]))
non_priority_trim_validations_summary <- non_priority_trim_validations_summary %>% pivot_longer(cols= everything(), names_to = "error", values_to = "count")
non_priority_trim_validations_summary <- non_priority_trim_validations_summary %>% filter(count !=0)

prioritised_main_validation_summary <-rbind(priority_main_validations_summary, non_priority_main_validations_summary, non_priority_trim_validations_summary) 
#Save Outputs----------------------------------------------------------------------------------------------
...
#                       dataframe=all_main_validation_summary,
