...
#i.e. checks for 2023 (item2511 + item2536) will be performed against prev years item2511
...
context_items_yr1 <-
  lapply(context_items, function (x)
    paste("o", x, sep = ""))
names(context_items_yr1) <-
  lapply(names(context_items), function (x)
    paste("o", x, sep = ""))
...
perr3_vars <-
  c("perr3", total_poultry, ototal_poultry, vototal_poultry)
...
# perr1_desc <-
#   "perr1_Old grass increased to more than all grass last year"
...
# perr11_desc <-
#   "perr11_Introduction of male cattle to holding with only female cattle (excluding bulls for service)"
# perr12_desc <-
#   "perr12_Introduction of female cattle not for breeding to holding with only male cattle"
# perr13_desc <- "perr13_Switch from ware to seed potatoes"
# perr14_desc <-
#   "perr14_Large shift in deer numbers or movement from zero or to zero"
...
yr <- combined_JAC %>% group_by(parish, holding) %>%
  select (
    parish,
    holding,
    survtype,
    submisType,
    any_of(as.character(context_items)),
    other_data,
    land_data,
    saf_data,
    contains("rps")
...
#merge datasets
context_errors <-
  inner_join(prev_yr, yr, by = c("parish", "holding"))
context_errors <-
  right_join(prev_2yr, context_errors, by = c("parish", "holding"))

all_context_errors <- context_errors %>%
  mutate(
    tillage = sum(get(total_crops_fallow), get(grass_lt5)),
    ...
    glass = sum(
      #item87,
      get(glass_strawb_tot),
      get(glass_rasp_tot),
      get(glass_blue_tot),
      #item2036,item2037,
      get(glass_bed_pot_plant_tot),
      get(glass_hardy_nurse_tot),
      #item6000,
      get(glass_unused_tot)
    ),
    oglass = sum(
      #oitem87,
      get(oglass_strawb_tot),
      get(oglass_rasp_tot),
      get(oglass_blue_tot),
      #oitem2036, oitem2037,
      get(oglass_bed_pot_plant_tot),
      get(oglass_hardy_nurse_tot),
      get(oglass_blue_tot)
    ),
    voglass = sum(
      #voitem87,
      get(voglass_strawb_tot),
      get(voglass_rasp_tot),
      get(voglass_blue_tot),
      #voitem2036, voitem2037,
      get(voglass_bed_pot_plant_tot),
      get(voglass_hardy_nurse_tot)
    ),
...
all_context_errors <- all_context_errors %>%
  mutate(
    #Grass check -------------------------------------------------------------------------
    #If total area has gone up or down by 10% and there has been an increase of grass greater than 5 years in the last year that does not account for the total of last year's rough grazings, fallow, other land and the difference in total land use area change.
    checkperr1 = 0.9 * get(ototal_area),
    perr1 = case_when(
      0.9 * get(oarea_own) < get(total_area) &
        get(total_area) < 1.1 * get(ototal_area) &
        othgrass - oothgrass > ograss + get(orough_graze) + (get(fland_lte5) + get(fland_gt5)) +
        get(oother_land) + 20 + max(get(total_area) - get(ototal_area), 0) ~ 1,
      TRUE ~ 0
    ),
    ...
    #if the total number of pigs is greater than 5 times or less than a fifth of last year's pigs and the land area has not as much as doubled
    perr2 = case_when((
      get(total_pig) > 5. * get(ototal_pig) &
        get(total_area) < 2 * get(ototal_area)
    )  |
      (
        get(total_pig) < 0.2 * get(ototal_pig) &
          get(total_area) > 0.5 * get(ototal_area)
      ) &
      ((get(total_pig) - get(ototal_pig)) > 10000 |
         (get(ototal_pig) - get(total_pig)) > 1000) &
      (
        get(total_pig) > 5. * get(vototal_pig) |
          get(total_pig) < 0.2 * get(vototal_pig)
      ) &
      ((get(total_pig) - get(vototal_pig)) > 10000 |
         get(vototal_pig) - get(total_pig) > 1000) &
      (
        get(ototal_pig) - get(total_pig) > 1.5 * (get(vototal_pig) - get(ototal_pig)) &
          get(ototal_pig) - get(total_pig) > 0 |
          get(ototal_pig) - get(total_pig) < 1.5 * (get(vototal_pig) - get(ototal_pig)) &
          get(ototal_pig) - get(total_pig)
      ) < 0  ~ 1,
    TRUE ~ 0
    ),
    ...
    #   Poultry check  --------------------------------------------------------
    #if total poultry is greater than 8 times or less than an eighth last year's poultry and the land area has not as much as doubled
    
    perr3 = case_when ((
      get(total_poultry) > 8. * get(ototal_poultry) &
        get(total_area) < 2 * get(ototal_area)
    ) |
      (
        get(total_poultry) < 0.125 * get(ototal_poultry) &
          get(total_area) > 0.5 * get(ototal_area)
      ) &
      (
        get(total_poultry) - get(ototal_poultry) > 150000 |
          (get(ototal_poultry) - get(total_poultry)) > 100000
      ) &
      (
        get(total_poultry) > 8. * get(vototal_poultry) |
          get(total_poultry) < 0.125 * get(vototal_poultry)
      ) &
      (get(total_poultry) - get(vototal_poultry)) > 150000 |
      (get(vototal_poultry) - get(total_poultry) > 100000) &
      (
        get(ototal_poultry) - get(total_poultry) > 1.5 * (get(vototal_poultry) -
                                                            get(ototal_poultry)) & get(ototal_poultry) - get(total_poultry) > 0 |
          get(ototal_poultry) - get(total_poultry) < 1.5 *
          (get(vototal_poultry) - get(ototal_poultry)) &
          get(ototal_poultry) - get(total_poultry) < 0
      ) ~ 1,
    TRUE ~ 0
    ),
    ...
    #Sheep check ------------------------------------------------------------------------------------------------------------------------------------------
    perr7 = case_when((
      get(total_sheep) > 10. * get(ototal_sheep) &
        get(total_area) < 2 * get(ototal_area)
    ) |
      (
        get(total_sheep) < 0.1 * get(ototal_sheep) &
          get(total_area) > 0.5 * get(ototal_area)
      ) &
      abs(get(total_sheep) - get(ototal_sheep)) > 5000 &
      (
        get(total_sheep) > 10. * get(vototal_sheep) |
          get(total_sheep) < 0.1 * get(vototal_sheep)
      ) &
      abs(get(total_sheep) - get(vototal_sheep)) > 5000 &
      (
        get(ototal_sheep) - get(total_sheep) > 1.5 * (get(vototal_sheep) - get(ototal_sheep)) &
          get(ototal_sheep) - get(total_sheep) > 0 |
          get(ototal_sheep) - get(total_sheep) < 1.5 * (get(vototal_sheep) -
                                                          get(ototal_sheep)) & get(ototal_sheep) - get(total_sheep) < 0
      ) ~ 1,
    TRUE ~ 0
    ),
    ...
    #Soft fruit check--------------------------------------------------------------------------------------------------------
    perr9 =  case_when((
      get(soft_fruits)  > 3. * get(osoft_fruits) |
        get(soft_fruits)  < 0.3 * get(osoft_fruits)
    ) &
      ((get(soft_fruits) - get(osoft_fruits)) > 10 |
         (get(osoft_fruits) - get(soft_fruits)) > 1) &
      (
        get(soft_fruits)  > 3. * get(vosoft_fruits) |
          get(soft_fruits)  < 0.3 * get(vosoft_fruits)
      ) &
      ((get(soft_fruits) - get(vosoft_fruits)) > 10 |
         (get(vosoft_fruits) - get(soft_fruits)) > 1) &
      (
        get(osoft_fruits) - get(soft_fruits)  > 1.5 * (get(vosoft_fruits) - get(osoft_fruits)) &
          get(osoft_fruits) - get(soft_fruits)  > 0 |
          get(osoft_fruits) - get(soft_fruits)  < 1.5 * (get(vosoft_fruits) -
                                                           get(osoft_fruits)) & get(osoft_fruits) - get(soft_fruits) < 0
      ) &
    ...
    perr10 = case_when((glass > 0.1 * oglass | glass < 0.1 * oglass) &
                         ...
                         (
                           oglass - glass > 1.5 * (voglass - oglass) &
                             oglass - glass > 0 |
                             oglass - glass < 1.5 * (voglass - oglass) & oglass - glass < 0
                         ) ~ 1,
                       TRUE ~ 0
    ),
    ...
    perr13 = case_when(
      get(total_area) < 1.1 * get(ototal_area)  &&
        get(oseed_potatoes) == 0 &
        get(voseed_potatoes)  == 0 & get(seed_potatoes) > 10
    ...
    perr14 = case_when((get(deer) >= 50 |
                          get(odeer) >= 50) &&
                         ((deer_ratio > 1.8) | (deer_ratio <= 0.5 & deer_ratio > 0)
                         ) ~ 1,
...
all_context_errors <- all_context_errors %>%
  mutate(
    perr15 = case_when(
      survtype == "Non-SAF" &
        (
          get(total_land) >= 2 * get(ototal_land) |
            get(total_land) < 0.5 * get(ototal_land)
        ) &
        (get(total_land) > 1000 |
           get(ototal_land) > 1000) ~ 1,
      TRUE ~ 0
    ),
    ...
    perr16 = case_when(
      survtype == "Non-SAF" &
        (
          get(rough_graze) >= 2 * get(orough_graze) |
            get(rough_graze) < 0.5 * get(orough_graze)
        ) &
        (get(rough_graze) > 1000 |
           get(orough_graze) > 1000) ~ 1,
      TRUE ~ 0
    )
...
errors <-
  all_context_errors %>% filter((
    sum(perr2, perr3, perr7, perr10, perr13, perr14) > 0 &
      survtype == "Non-SAF"
  ) |
    (sum(perr2, perr3, perr7, perr10, perr14) > 0 &
       survtype == "SAF"))
...
perr1 <-
  perr1 %>%  mutate(
    error = ifelse(perr1 == 1, "Error", "No Error"),
...
perr2 <-
  perr2 %>%  mutate(
    error = ifelse(perr2 == 1, "Error", "No Error"),
    difference1 = ifelse(
      get(total_pig) - get(ototal_pig) < 0,-0.5 * (get(total_pig) - get(ototal_pig)),
      get(total_pig) - get(ototal_pig)
    ),
    difference2 = ifelse(
      get(total_pig) - get(vototal_pig) < 0,-0.5 * (get(total_pig) - get(vototal_pig)),
      get(total_pig) - get(vototal_pig)
    )
...
# check range of differences first
range(perr2$difference1)
range(perr2$difference2)
#filter for differences>100
perr2 <-
  perr2 %>% filter(abs(difference1) & abs(difference2)  >= 100)
...
perr3 <-
  perr3 %>%  mutate(
    error = ifelse(perr3 == 1, "Error", "No Error"),
    difference = ifelse(
      get(total_poultry) - get(ototal_poultry) < 0,-0.5 * get(total_poultry) -
        get(ototal_poultry),
      get(total_poultry) - get(ototal_poultry)
    )
  )
...
# check range of differences first
range(perr3$difference)

#filter for difference >=10,000
perr3 <- perr3 %>% filter(abs(difference) >= 10000)
...
perr7 <-
  perr7 %>%  mutate(
    error = ifelse(perr7 == 1, "Error", "No Error"),
    difference1 = ifelse(
      get(total_sheep) - get(ototal_sheep) < 0,-0.5 * (get(total_sheep) - get(ototal_sheep)),
      get(total_sheep) - get(ototal_sheep)
    ),
    difference2 = ifelse(
      get(total_sheep) - get(vototal_sheep) < 0,-0.5 * (get(total_sheep) - get(vototal_sheep)),
      get(total_sheep) - get(vototal_sheep)
    )
  )
#filter for errors
...
# check range of differences first
range(perr7$difference1)
range(perr7$difference2)

#filter for difference >=500
perr7 <-
  perr7 %>% filter(abs(difference1) >= 500 & abs(difference2 >= 500))
...
perr9 <-
  perr9 %>%  mutate(
    error = ifelse(perr9 == 1, "Error", "No Error"),
    difference = ifelse(
      get(soft_fruits)  - get(osoft_fruits)  < 0,-0.5 * (get(soft_fruits) - get(osoft_fruits)),
      get(soft_fruits) - get(osoft_fruits)
    )
  )
...
perr10 <-
  perr10 %>%  mutate(
    error = ifelse(perr10 == 1, "Error", "No Error"),
    difference = ifelse(glass - oglass < 0,-0.5 *
                          (glass - oglass), glass - oglass)
  )
...
perr13 <-
  perr13 %>%  mutate(error = ifelse(perr13 == 1, "Error", "No Error"))
...
perr14 <-
  perr14 %>%  mutate(error = ifelse(perr14 == 1, "Error", "No Error"))
...
perr15 <-
  perr15 %>%  mutate(error = ifelse(perr15 == 1, "Error", "No Error"))
...
perr16 <-
  perr16 %>%  mutate(error = ifelse(perr16 == 1, "Error", "No Error"))
...
#validations checks that have been run on the data in this session
relevant_errors <-
  names(errors)[names(errors) %in% context_error_list]


#dataframe of all errors that have been filtered for thresholds
error_filter <- bind_rows(sapply(relevant_errors, get))

error_filter <- error_filter %>% ungroup() %>%
  select(submisType, starts_with("perr")) %>% group_by(submisType)

#summary of context errors by submisType
context_error_summary <-
  error_filter %>% summarize(across(everything(), sum, na.rm = TRUE)) %>% ungroup()
context_error_summary <- cbind(context_error_summary,
                               total_cases_with_errors =  rowSums(context_error_summary[relevant_errors]))

context_error_summary <-
  as.data.frame(t(context_error_summary)) %>%
  janitor::row_to_names(1)
colnames(context_error_summary) <-
  paste("SubmisType", colnames(context_error_summary))



#clean environment
rm(combined_JAC, prev_2yr, prev_yr, yr, context_errors)
##----Output for Ops--------------------------------------------------------------------------------------------
errors <- bind_rows(sapply(relevant_errors, get))

#count number of errors for each holding (exclude parish and holding, first two columns)
errors$total_errors_per_case <-
  rowSums(errors[names(errors) %in% context_error_list], na.rm = TRUE)
...
errors <-
  errors %>% select(parish, holding, starts_with("perr"), total_errors_per_case)

#remove columns with NAs only from each dataframe in holding_list
holding_list <- remove_NA(errors)
...
#                       dataframe=context_error_summary,
