...
combined_data_2023<-combined_data_excl %>% 
  rowwise() %>% 
  mutate(
  item50=ifelse(survtype=="SAF_only", sum(c(item46,item47,item48,item49), na.rm=TRUE), item50))

check<-combined_data_2023 %>% 
  select(parish, holding, item46, item47, item48, item49, item48_15, item49_15, strata, wood9999, item9999,  wood_prop, item48_23, item49_23, flag, item50, survtype, land_data)
